...
GROUP BY `brand`.`id`")->result();
	...
	function favoritebrands()
	{
		$brands=$this->db->query("SELECT * FROM `brand`")->result();

		for($i=0;$i<sizeof($brands);$i++)
		{
			$brands[$i]->like=$this->getlike($brands[$i]->id);
		}
		return $brands;
...
GROUP BY `store`.`id`")->result();
		...
		{
			$query[$i]->offer=$this->getofferbystore($query[$i]->storeid);
			$query[$i]->newin=$this->getnewinbystore($query[$i]->storeid);
		}

		return $query;
	}
	
    function getofferbystore($storeid)
    {
    	$query=$this->db->query("SELECT `offers`.`header`,`offers`.`description` FROM `offers` LEFT OUTER JOIN `storeoffers` ON `offers`.`id`=`storeoffers`.`offerid` WHERE `storeoffers`.`storeid`='$storeid'")->result();
    	return $query;
    }
    
    
    function getnewinbystore($storeid)
    {
    	$query=$this->db->query("SELECT `newin`.`image`,`newin`.`description` FROM `newin` LEFT OUTER JOIN `storenewin` ON `newin`.`id`=`storenewin`.`newinid` WHERE `storenewin`.`storeid`='$storeid'")->result();
    	return $query;
    }
...
WHERE `storenewin`.`storeid`='$storeid'")->result();
	...
	function searchstorepage($name)
	{
		$query=$this->db->query("SELECT `store`.`id`AS `storeid`,`store`. `name`,`store`. `city`,`city`.`name` as `cityname`, `store`.`brand` AS `brandid`,`brand`.`name` AS `brandname`,`brand`.`logo` as `brandlogo`,`store`. `format`,`store`. `mall`,`mall`.`name` as `mallname`, `store`.`floor`,`floor`.`floorno` AS `floorname`,`mall`. `address`,`store`.`address` AS `storeaddress`, `mall`.`location`,`location`.`name` AS `locationname`,`mall`. `latitude`, `mall`.`longitude`,`store`.`contactno`, `store`.`email`,`store`.`description`,AVG(`storerating`.`rating`) as `rating`
FROM `storecategory`
LEFT OUTER JOIN `store` ON `store`. `id`=`storecategory`.`storeid`
LEFT OUTER JOIN `storerating` ON `storerating`.`store`=`store`.`id`
LEFT OUTER JOIN `brand` ON `store`. `brand`=`brand`.`id`
