...
WHERE `brandcategory`.`categoryid`='$id'")->result();
	...
	{
		$query=$this->db->query("SELECT `brand`.`id`, `brand`.`name`, `brand`.`pricerange`,`pricerange`.`range` AS `rangename`, `brand`.`video`, `brand`.`description`, `brand`.`facebookpage`, `brand`.`website`, `brand`.`twitterpage`, `brand`.`logo` FROM `brand` LEFT OUTER JOIN `pricerange` ON `pricerange`.`id`=`brand`.`pricerange` WHERE `brand`.`name` LIKE '%$search%' OR `brand`.`description` LIKE '%$search%'
")->result();
		return $query;
	}
