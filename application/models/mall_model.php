...
//            print_r($locationquery);
//            echo $locationquery->id;
...
//		}
    ...
    function searchbymall($city,$search)
	{
		if($city==0)
		{
		$newcity=" WHERE `mall`.`name` LIKE '%$search%' OR `mall`.`address` LIKE '%$search%'";
		}else{
		$newcity=" WHERE `mall`.`city` ='$city' AND `mall`.`name` LIKE '%$search%' OR `mall`.`address` LIKE '%$search%'";
		}
		$sqlqry="SELECT `mall`.`id`, `mall`.`name`, `mall`.`address`, `mall`.`location`, `mall`.`latitude`, `mall`.`longitude`, `mall`.`contactno`, `mall`.`parkingfacility`, `mall`.`cinema`, `mall`.`restaurant`, `mall`.`entertainment`, `mall`.`website`, `mall`.`email`, `mall`.`logo` ,`city`.`name` AS `cityname`,`location`.`name` AS `locationname` FROM `mall` LEFT OUTER JOIN `city` ON `city`.`id`=`mall`.`city` LEFT OUTER JOIN `location` ON `location`.`id`=`mall`.`location`".$newcity;
		$query=$this->db->query($sqlqry)->result();
		return $query;
...
GROUP BY `store`.`id`")->result();
	...
	{
		$query=$this->db->query("SELECT `store`.`name`,`store`.`mall`,`store`.`address`,`store`.`location`,`store`.`latitude`,`store`.`longitude`,`storeoffers`.`offerid`,`storeoffers`.`storeid`,`offers`.`header`,`offers`.`description`,`offers`.`from`,`offers`.`to`
FROM `store`
